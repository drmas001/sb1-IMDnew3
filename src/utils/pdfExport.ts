...
export const exportToPDF = ({ patients, consultations, appointments, activeTab, dateFilter }: ExportData) => {
  try {
    ...
    currentY += 10;
    doc.setFontSize(12);
    doc.text(`Generated on: ${format(new Date(), 'PPpp')}`, pageWidth / 2, currentY, { align: 'center' });
    
    if (dateFilter.startDate && dateFilter.endDate) {
      currentY += 7;
      doc.text(
        `Period: ${format(new Date(dateFilter.startDate), 'PP')} to ${format(new Date(dateFilter.endDate), 'PP')}`,
        pageWidth / 2,
        currentY,
        { align: 'center' }
      );
